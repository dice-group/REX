...
public class ExperimentRunner implements Runnable {
	...
	private boolean eqDist;
	private int i;

	public ExperimentRunner(String index, String prop, String domain, int maxPairs, int testPages,
			boolean quality, boolean eqDist, int i) throws MalformedURLException {
	...
	private void setGoldenRules(String prop, String domain) {
		...
		if (domain.equals("http://www.imdb.com/name/")) {
			if (prop.equals("director")) {
				this.goldenRuleL = new XPathRule(
						"//*[@id='filmo-head-director']/following-sibling::*[1]/DIV/B/A/text()");
				this.goldenRuleR = new XPathRule(
						"//*[contains(text(),'Take The Quiz!')]/../SPAN[1]/A[1]/text()[1]");
			}
			if (prop.equals("starring")) {
	...
	public void run() {
		...
		log.info("Execution Time: " + now+ " |I|="+this.i);
		...
		if (this.checkQuality) {
			...
			// List<Page> notRepresentedPagesL =
			// samplerL.getNonRepresentedPages();
			// List<Page> notRepresentedPagesR =
			// samplerR.getNonRepresentedPages();
			//
			// log.debug("Test on learning pages");
			//
			// QualityEvaluator representedL = testResultRule(ruleL,
			// this.goldenRuleL, representedPagesL);
			// QualityEvaluator representedR = testResultRule(ruleR,
			// this.goldenRuleR, representedPagesR);
			// QualityEvaluator notRepresentedL = testResultRule(ruleL,
			// this.goldenRuleL,
			// notRepresentedPagesL);
			// QualityEvaluator notRepresentedR = testResultRule(ruleR,
			// this.goldenRuleR,
			// notRepresentedPagesR);
			QualityEvaluator representedL = testResultRule(ruleL, this.goldenRuleL, this.testing);
			QualityEvaluator representedR = testResultRule(ruleR, this.goldenRuleR, this.testing);

			log.info(this.print(this.indexName, this.propertyName, "Left", trainingPages.size()
					+ "", this.testing.size() + "", this.testing.size() + "",
					representedL.getPrecision() + "", representedL.getRecall() + "",
					representedL.getF() + "", "", "", ""));

			log.info(this.print(this.indexName, this.propertyName, "Right", trainingPages.size()
					+ "", this.testing.size() + "", this.testing.size() + "",
					representedR.getPrecision() + "", representedR.getRecall() + "",
					representedR.getF() + "", "", "", ""));
		}
	}
	...
	public static void main(String[] args) throws MalformedURLException {
		...
		if (args.length > 5)
			equalDistribution = Boolean.parseBoolean(args[5]);
		int i = 0;
		if (args.length > 6)
