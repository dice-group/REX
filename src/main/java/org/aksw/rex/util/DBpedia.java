...
/**
 * class used sparql the official DBpedia endpoitn
 * 
 * @author r.usbeck
 * 
 */
public class DBpedia {
	...
	/**
	 * used to extract the topClasses
	 * 
	 * @return
	 */
	private ArrayList<ArrayList<String>> getTopClasses() {
		String query = "PREFIX rdf:<http://www.w3.org/1999/02/22-rdf-syntax-ns#> " + "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> " + "PREFIX owl:<http://www.w3.org/2002/07/owl#> " + "SELECT ?s (COUNT(?i) AS ?count ) WHERE { " + "    ?s  rdf:type owl:Class. " + "    OPTIONAL { "
				+ "    ?o  rdfs:subClassOf ?s. " + "    } " + "    ?i rdf:type ?s. " + "    FILTER(!bound(?o)&& STRSTARTS(STR(?s), \"http://dbpedia.org/ontology/\")) " + "} " + "GROUP BY ?s " + "ORDER BY DESC(?count) " + "LIMIT 10";

		return askDbpedia(query);
	}

	/**
	 * @return a matrix of query results where each row represents a result and
	 *         each column a variable projection of the query
	 * 
	 */
	private ArrayList<ArrayList<String>> askDbpedia(String query) {
