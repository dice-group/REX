...
public class RexController {
...

	{
		...
		if (!extractionRules.isEmpty()) {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter("ntFiles/goldstandard/withOutConsistency" + domain.toExternalForm().replaceAll("//", "").replaceAll("/", "") + ".txt"));
	...
	public static void main(String[] args) throws Exception {
		...
		// d.add(new ControllerData("espnfc-player-index/",
		// "http://dbpedia.org/ontology/team", "http://espnfc.com/player/_/id/",
		// "//*[@class='profile']/H1/text()",
		// "//*[contains(text(),'Teams')]/../UL/LI/A/text()"));
		...
		for (ControllerData ds : d) {
			try {
				...
				BufferedWriter bw = new BufferedWriter(new FileWriter("ntFiles/goldstandard/" + domain.toExternalForm().replaceAll("//", "").replaceAll("/", "") + ".txt"));
				for (Quadruple<Node, Node, Node, String> q : quads) {
					bw.write("<" + q.first.getURI() + "> <" + q.second().getURI() + "> <" + q.third().getURI() + "> <" + q.fourth() + ">.\n");
				}
				bw.flush();
